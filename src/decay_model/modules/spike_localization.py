...
class SpikeLocalizationEncoder(nn.Module):
    ...
    def __init__(
        self, waveforms_dim: int, capacity1: int = 500, capacity2: int = 200, dropout1: float = 0, dropout2: float = 0, device: str = "cuda"
    ):
...
class SpikeLocalizationDecoder(nn.Module):
    ...
    def __init__(
        self, exps: torch.Tensor, ch_locs: torch.Tensor, device: str = "cuda"
    ):
    ...
    def forward(self, sampled_n_loc: torch.Tensor, exp_ids: torch.Tensor) -> torch.Tensor:
        ...
        recon_amps = -torch.exp(distances * self.exps[exp_ids, 1].unsqueeze(-1) + self.exps[exp_ids, 0].unsqueeze(-1))
        return recon_amps

class SpikeLocalizationVAE(nn.Module):

    def __init__(
        self, waveforms_dim: int, exps: torch.Tensor, ch_locs: torch.Tensor, capacity1: int = 500, capacity2: int = 200, dropout1: float = 0, dropout2: float = 0, device: str = "cuda"
    ):
        super(SpikeLocalizationVAE, self).__init__()

        self.encoder = SpikeLocalizationEncoder(
            waveforms_dim, capacity1, capacity2, dropout1, dropout2, device
        )
        self.decoder = SpikeLocalizationDecoder(exps, ch_locs, device)

    def reparameterize_normal(self, mu: torch.Tensor, var: torch.Tensor) -> torch.Tensor:
        std = var.sqrt()
        eps = torch.randn_like(std)
        return eps * std + mu

    def forward(self, waveforms: torch.Tensor, exp_ids: torch.Tensor, training: bool) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
class SpikeLocalizationLossFunc(nn.Module):
    ...
    def __init__(self, prior_var: float, device="cuda"):
        ...
        self.prior_var = torch.cat((torch.tensor(prior_var**2).unsqueeze(0), torch.tensor(prior_var**2).unsqueeze(0), torch.tensor(prior_var**2).unsqueeze(0))).view(1, -1).to(device)
        self.to(device)

    def kl_divergence_normal(self, mu_q: torch.Tensor, var_q: torch.Tensor, mu_p: torch.Tensor, var_p: torch.Tensor) -> torch.Tensor:
        return torch.sum(0.5 * (torch.log(var_p) - torch.log(var_q)) + torch.div(var_q + (mu_q - mu_p) ** 2, 2 * var_p - 0.5))

    def forward(self, recon_amps: torch.Tensor, amps: torch.Tensor, xyz_mu: torch.Tensor, xyz_var: torch.Tensor) -> torch.Tensor:
        batch_size = recon_amps.shape[0]
        xyz_muq = torch.zeros_like(xyz_mu)
        xyz_varq = torch.ones_like(xyz_var) * self.prior_var
        loss = (torch.sum(F.mse_loss(recon_amps, amps[:, :, 0], reduction="none") * amps[:, :, 1]) + self.kl_divergence_normal(xyz_mu, xyz_var, xyz_muq, xyz_varq)) / batch_size
        return loss
