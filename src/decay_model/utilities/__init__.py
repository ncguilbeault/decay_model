...
def load_channel_positions_from_xml(
    xml_path: str,
    ...
    assert len(xpos) == len(
        ypos
    ), "The number of x and y channel positions must be the same"
    assert all(key in ypos for key in xpos) and all(
        key in xpos for key in ypos
    ), "The channel keys in x and y positions must match"
...
def pad_channel_positions(
    ...
    assert (
        x_spacing is not None or x_coords.shape[0] > 1
    ), "If x_spacing is not provided, there must be more than one unique x coordinate to infer spacing"

    assert (
        y_spacing is not None or y_coords.shape[0] > 1
    ), "If y_spacing is not provided, there must be more than one unique y coordinate to infer spacing"
    ...
    return torch.vstack(
        (channel_positions, shifted_positions.view(-1, channel_positions.shape[1]))
    )
